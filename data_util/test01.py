...
def remove_stop_word(dataset, stop_words):
    ...
    return [word for word in dataset if len(word) > 1 and word not in stop_words]
    # print(len(all_words), all_words[:20])
    # pd.DataFrame(all_words).to_csv('data/deep_clean_word_finally1.txt', index=False)
...
def deep_clean_words(clean_words):
    ...
    for w in tqdm(deep_clean_ws2):
        w = w.split('/')
        deep_clean_ws.extend(w)
    for w in tqdm(deep_clean_ws):
        w = w.split('.')
        deep_clean_ws1.extend(w)
    for w in tqdm(deep_clean_ws1):
        w = w.split('\'')
        deep_clean_ws3.extend(w)
    stop_words = get_stop_word()
    stop_words = stop_words + ['<', '>', '\'', '\'s', '+/-', '/', '--', '°c', '``', '►', '·', '×', '”', '||', '..',
                               '//', '`']
    return remove_stop_word(deep_clean_ws3, stop_words)
