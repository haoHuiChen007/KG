...
class BertReModel(BertPreTrainedModel):
    def __init__(self, config, params):
        ...
        # sequence tagging
        self.sequence_tagging_sub = MultiNonLinearClassifier(config.hidden_size * 2, self.seq_tag_size,
                                                             params.drop_prob)
        self.sequence_tagging_obj = MultiNonLinearClassifier(config.hidden_size * 2, self.seq_tag_size,
                                                             params.drop_prob)
    ...
    def forward(
        ...
        if ensure_rel:
            # (bs, h)
            h_k_avg = self.masked_avg_pool(sequence_output, attention_mask)
            # (bs, rel_num)
            self.rel_pred = self.rel_judgement(h_k_avg)
        ...
        if ensure_rel and seq_tags is None:
            # (bs, rel_num)
            rel_pred_onehot = torch.where(torch.sigmoid(self.rel_pred) > rel_threshold,
                                          torch.ones(self.rel_pred.size(), device=self.rel_pred.device),
                                          torch.zeros(self.rel_pred.size(), device=self.rel_pred.device))

            # if potential relation is null
            for idx, sample in enumerate(rel_pred_onehot):
                if 1 not in sample:
                    # (rel_num,)
                    max_index = torch.argmax(self.rel_pred[idx])
                    sample[max_index] = 1
        ...
        if seq_tags is not None:
            ...
            if ensure_rel:
                ...
                loss_rel = loss_func(self.rel_pred, rel_tags.float())
...
if __name__ == '__main__':
    ...
    model = BertReModel.from_pretrained(config=bert_config,
