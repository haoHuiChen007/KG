...
from metrics import tag_mapping_corres
...
def evaluate(model, data_iterator, params, ex_params, mark='Val'):
    ...
    for batch in tqdm(data_iterator, unit='Batch', ascii=True):
        ...
        # inference
        with torch.no_grad():
            # , pre_corres, xi, pred_rels
            pred_seqs = model.forward(input_ids, attention_mask=attention_mask,
                                      ex_params=ex_params)
        ...
        # for p, t, l in zip(pred_seqs, input_ids, batch_len):
